if EXM then
	return print("ERROR: Can't enable ZONES cos EXM is already enabled!")
end
EXM = {}
local EXM_DATA = EXM
...
local function getZone(player, x,y)
	...
	if square then
		...
		if EXM_DATA.CustomServerFn then
			local custom_str = EXM_DATA.CustomServerFn(square,x,y)
...
local function getCoords()
	...
	if player then
		...
		textManager:DrawString(UIFont.Large, screenX, screenY + 30, cache_count, 0.1, 0.8, 1, 1);
	end
end
